...
| F1 ↑ | librivad | aishell1 | ten_official | picovoice |
|---|---|---|---|---|
| silero | **0.966** | 0.878 | **0.937** | **0.959** |
| tenvad | 0.962 | **0.891** | 0.931 | 0.912 |

*librivad*（en）

| 模型 | F1 | 准确率 | 精确率 | 召回率 | AUC | RTF | CMM(MB) | OS(MB) |
|---|---|---|---|---|---|---|---|---|
| silero | **0.966** | **0.944** | **0.969** | 0.964 | **0.977** | **0.029** | 0.9 | 45 |
| tenvad | 0.962 | 0.935 | 0.956 | **0.968** | 0.975 | 0.097 | **0.5** | **4** |

*aishell1*（zh）

| 模型 | F1 | 准确率 | 精确率 | 召回率 | AUC | RTF | CMM(MB) | OS(MB) |
|---|---|---|---|---|---|---|---|---|
| tenvad | **0.891** | **0.814** | 0.998 | **0.805** | **0.908** | 0.097 | **0.5** | **4** |
| silero | 0.878 | 0.794 | **1.000** | 0.783 | 0.905 | **0.029** | 0.9 | 45 |

*ten_official*（en）

| 模型 | F1 | 准确率 | 精确率 | 召回率 | AUC | RTF | CMM(MB) | OS(MB) |
|---|---|---|---|---|---|---|---|---|
| silero | **0.937** | **0.905** | 0.938 | **0.937** | **0.956** | **0.029** | 0.9 | 45 |
| tenvad | 0.931 | 0.898 | **0.941** | 0.922 | 0.949 | 0.097 | **0.5** | **4** |

*picovoice*（en）

| 模型 | F1 | 准确率 | 精确率 | 召回率 | AUC | RTF | CMM(MB) | OS(MB) |
|---|---|---|---|---|---|---|---|---|
| silero | **0.959** | **0.986** | **0.990** | 0.930 | **0.997** | **0.029** | 0.9 | 45 |
| tenvad | 0.912 | 0.966 | 0.850 | **0.983** | 0.996 | 0.097 | **0.5** | **22** |
<!-- /RESULTS:vad -->
...
| 回环CER ↓ | aishell3 | zh_long | zh_hardcase |
|---|---|---|---|
| zipvoice | **4.98%** | **0.58%** | — |
| pocket_tts_zh_en | 8.46% | 0.68% | **2.46%** |
| melotts | 8.22% | 1.64% | — |
| pocket_tts_zh_en_pinyin | 11.95% | 1.11% | 4.58% |
| cosyvoice2 | 13.43% | 1.84% | — |
| cosyvoice2-ts4 | 20.90% | — | — |
| kokoro | 70.59% | — | — |

*aishell3*（zh）

| 模型 | 回环CER | GT锚点CER | 替换率(GT) | 删除率(GT) | 插入率(GT) | 替换率 | 删除率 | 插入率 | 成功率 | 均长 | 时长比 | RTF(含加载) | RTF |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| zipvoice | **4.98%** | — | — | — | — | **4.90%** | **0.08%** | **0.00%** | **100.0%** | 1.30s | 0.588 | **3.18** | 0.155 |
| melotts | 8.22% | — | — | — | — | 7.63% | 0.50% | 0.08% | 100.0% | 1.93s | 0.968 | — | **0.140** |
| pocket_tts_zh_en | 8.46% | — | — | — | — | 7.30% | 1.08% | 0.08% | 100.0% | 1.20s | 0.548 | — | 0.737 |
| pocket_tts_zh_en_pinyin | 11.95% | — | — | — | — | 10.04% | 1.74% | 0.17% | 100.0% | 1.25s | 0.529 | — | 0.860 |
| cosyvoice2 | 13.43% | — | — | — | — | 5.22% | 8.21% | 0.00% | 90.0% | 1.92s | 0.825 | 556.59 | 3.242 |
| cosyvoice2-ts4 | 20.90% | — | — | — | — | 6.72% | 14.18% | 0.00% | 75.0% | 2.29s | 0.980 | — | — |
| kokoro | 70.59% | — | — | — | — | — | — | — | — | — | — | — | 10.094 |
| GT | — | 2.90% | 2.66% | 0.25% | 0.00% | — | — | — | — | — | — | — | — |

*zh_long*（zh）

| 模型 | 回环CER | 替换率 | 删除率 | 插入率 | 成功率 | 均长 | RTF(含加载) | RTF |
|---|---|---|---|---|---|---|---|---|
| zipvoice | **0.58%** | **0.58%** | **0.00%** | **0.00%** | **100.0%** | 10.98s | **0.59** | 0.196 |
| pocket_tts_zh_en | 0.68% | 0.63% | 0.05% | 0.00% | 100.0% | 10.73s | — | 0.699 |
| pocket_tts_zh_en_pinyin | 1.11% | 0.97% | 0.14% | 0.00% | 100.0% | 11.20s | — | 0.762 |
| melotts | 1.64% | 1.59% | 0.05% | 0.00% | 100.0% | 12.46s | — | **0.134** |
| cosyvoice2 | 1.84% | 1.10% | 0.28% | 0.46% | 100.0% | 13.53s | 1.20 | — |

*librispeech*（en）

| 模型 | 替换率(GT) | 删除率(GT) | 插入率(GT) | 替换率 | 删除率 | 插入率 | 成功率 | 均长 | 时长比 | RTF(含加载) | 回环WER | GT锚点WER | RTF |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| cosyvoice2 | — | — | — | 26.84% | 5.41% | 3.25% | **100.0%** | 11.47s | 1.342 | **1.31** | 35.50% | — | — |
| GT | 1.48% | 0.33% | 0.23% | — | — | — | — | — | — | — | — | 2.04% | — |
| melotts | — | — | — | 8.87% | **0.75%** | 1.80% | 100.0% | 8.85s | 1.178 | — | **11.43%** | — | **0.196** |
| zipvoice | — | — | — | **8.50%** | 8.52% | **0.91%** | 100.0% | 5.07s | 0.779 | 1.63 | 17.93% | — | 0.221 |

*ljspeech*（en）

| 模型 | 替换率(GT) | 删除率(GT) | 插入率(GT) | 替换率 | 删除率 | 插入率 | 成功率 | 均长 | 时长比 | RTF(含加载) | 回环WER | GT锚点WER | RTF |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| cosyvoice2 | — | — | — | 5.69% | 0.60% | 1.80% | **100.0%** | 7.61s | 1.268 | 1.45 | 8.08% | — | — |
| melotts | — | — | — | 11.50% | 0.89% | 3.49% | 100.0% | 7.79s | 1.223 | — | 15.88% | — | **0.158** |
| zipvoice | — | — | — | **2.92%** | **0.24%** | 2.71% | 100.0% | 5.83s | 0.910 | **1.41** | 5.87% | — | 0.204 |
| GT | 3.19% | 0.33% | 2.29% | — | — | — | — | — | — | — | — | 5.81% | — |
| pocket_tts_zh_en | — | — | — | 5.04% | 0.92% | 1.88% | 100.0% | 5.46s | 0.844 | — | 7.84% | — | 0.709 |
| pocket_tts_zh_en_pinyin | — | — | — | 3.16% | 1.52% | **0.74%** | 100.0% | 5.60s | 0.853 | — | **5.42%** | — | 0.750 |

*zh_hardcase*（zh）

| 模型 | 回环CER | 替换率 | 删除率 | 插入率 | 成功率 | 均长 | RTF |
|---|---|---|---|---|---|---|---|
| pocket_tts_zh_en | **2.46%** | **1.52%** | **0.69%** | **0.26%** | **100.0%** | 4.81s | **0.719** |
| pocket_tts_zh_en_pinyin | 4.58% | 2.26% | 0.77% | 1.55% | 100.0% | 5.52s | 0.761 |
<!-- /RESULTS:tts -->
