#!/usr/bin/env python3
"""Pocket-TTS 中英双语拼音版（HY-2012/pocket-tts-zh-en-pinyin.AXERA）载入一次批量合成（板端）。

模型目录布局（HF 推理包）：
    <model_dir>/models/{flow_step_windowed.onnx, flow_ar_step.onnx,
        flow_net_step_fp32.axmodel, mimi_conv_step.axmodel,
        mimi_split/mimi_transformer_step.onnx, encoder/step_encoder_40f.axmodel,
        tokens.txt, chn_jpn_yue_eng_ko_spectok.bpe.model, Vivian.wav}
    <model_dir>/board/pocket_tts_axera.py

int8-only 混合路线：FlowLM 用供应商 int8 原图在 CPU；flow_net/mimi_conv/编码器走 NPU。
零样本：参考音色固定用包内 Vivian.wav；同一进程内音色前缀缓存复用。

用法: python pocket_tts_zh_en_pinyin_batch.py --dataset aishell3 [--limit N]
"""
import argparse
import sys
import time
...
from common import load_config, TTS_DATASETS  # noqa: E402

CFG = load_config()
MODEL_DIR = Path(CFG["paths"]["model_root"]) / CFG["models"]["tts"]["pocket_tts_zh_en_pinyin"]["dir"]
...
def build_runtime(threads=4, prefill_threads=8, npu_flow_net=True):
    return PocketTTS(
        str(MODELS), str(MODELS), cpu_model_dir=str(MODELS / "cpu"),
        mimi_split_dir=str(MODELS / "mimi_split"),
        threads=threads, flow_window=512,
        npu_flow_net=npu_flow_net, npu_mimi_conv=True,
...
def main():
    ...
    ap.add_argument("--npu-flow-net", type=int, default=1,
                    help="0 = flow_net 回 CPU 原图（与 vendor 逐位一致，慢）")
    ...
    out_dir = (Path(CFG["paths"]["data_root"]) / "benchmark" / "tts" / "syn"
               / f"pocket_tts_zh_en_pinyin_{args.dataset}")
    out_dir.mkdir(parents=True, exist_ok=True)

    t_load = time.perf_counter()
    tts = build_runtime(args.threads, args.prefill_threads, bool(args.npu_flow_net))
    frontend = TextFrontend(str(MODELS / "tokens.txt"),
                            str(MODELS / "chn_jpn_yue_eng_ko_spectok.bpe.model"))
    ref = load_reference(str(MODELS / "Vivian.wav"))
    print(f"Pocket-TTS zh-en-pinyin: 载入完成 {time.perf_counter() - t_load:.1f}s, "
          f"{len(rows)} 条待合成（参考音色 Vivian.wav, npu_flow_net={args.npu_flow_net}）")
    ...
    print(f">>> pocket_tts_zh_en_pinyin/{args.dataset}: 合成 {ok}/{len(rows)} 条 -> {out_dir}  "
